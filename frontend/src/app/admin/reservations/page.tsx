...
export default function AdminReservations() {
      ...
      try {
        ...
        if (json.success) {
          const dataWithEditState = json.data.map((res: any) => {
            const timeString = new Date(res.bookingDate).toLocaleTimeString(
              [],
  ...
  const handleCancel = (id: string) => {
    setReservations(
      reservations.map((res) =>
        res.id === id
          ? { ...res, isEditing: false, displayTime: res.originalTime }
          : res,
      ),
    );
  };
          ...
          <div className="bg-white border border-gray-300 rounded-lg p-4 shadow-inner min-h-[300px]">
            {reservations.length > 0 ? (
              reservations.map((res) => (
                <div
                  key={res.id}
                  className="relative flex flex-col sm:flex-row justify-between items-center bg-[#F9F9F9] p-5 border border-gray-200 rounded-lg mb-4 shadow-sm gap-4"
                >
