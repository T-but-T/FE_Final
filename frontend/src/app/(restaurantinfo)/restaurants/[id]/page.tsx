...
export default function ReservationPage({ params }: PageProps) {
  ...
  const handleReservation = async () => {
    try {
      ...
      const isWithinHours =
        reserveTime >= restaurant.openTime &&
        reserveTime <= restaurant.closeTime;
      if (!isWithinHours)
        return alert(
          `Sorry, this restaurant is open from ${restaurant.openTime} to ${restaurant.closeTime}`,
        );

      const bookingDateTime = new Date(
        `${reserveDate}T${reserveTime}:00`,
      ).toISOString();

      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/restaurants/${id}/reservations`,
        {
          method: "POST",
          headers: {
  ...
  if (loading)
    return (
      <div className="p-24 text-center mt-20 text-xl font-sans">
        Loading restaurant details...
      </div>
    );
  if (!restaurant)
    return (
      <div className="p-24 text-center mt-20 text-xl text-red-500 font-sans">
