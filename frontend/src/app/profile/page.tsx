...
export default function ProfilePage() {
        ...
        if (userData.success) {
          ...
          if (resData.success) {
            const mappedReservations = resData.data.map((r: any) => ({
              ...
              time: new Date(r.bookingDate).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
                hour12: true,
              }),
              originalTime: new Date(r.bookingDate).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
                hour12: true,
              }),
  ...
  const handleCancel = (id: string) => {
    setReservations(
      reservations.map((res) =>
        res.id === id
          ? { ...res, isEditing: false, time: res.originalTime }
          : res,
      ),
    );
  };
        ...
        reservations.map((res) =>
          res.id === id
            ? {
                ...
                rawDate: updatedDate.toISOString(),
              }
            : res,
        ),
      );
        ...
        <div className="w-full max-w-4xl bg-[#E5E5E5] rounded-xl p-12 relative shadow-lg">
          ...
          <div className="space-y-4 mb-10 text-lg">
            <p className="font-semibold text-[#4F4F4F]">
              Username:{" "}
              <span className="ml-4 text-black font-medium">{user.name}</span>
            </p>
            <p className="font-semibold text-[#4F4F4F]">
              Email:{" "}
              <span className="ml-4 text-black font-medium">{user.email}</span>
            </p>
            <p className="font-semibold text-[#4F4F4F]">
              Tel:{" "}
              <span className="ml-4 text-black font-medium">{user.tel}</span>
            </p>
          </div>
